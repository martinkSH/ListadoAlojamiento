...
export default async function HotelDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f8f6', fontFamily: "'DM Mono', 'Courier New', monospace" }}>
      <div style={{ background: '#1a1a1a', padding: '12px 24px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <Link href={`/hoteles/${params.id}/editar`} style={{ background: '#2a2a2a', color: '#fff', fontSize: '11px', padding: '5px 12px', borderRadius: '6px', textDecoration: 'none' }}>
            Editar
          </Link>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '24px 20px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            <thead>
              <tr style={{ background: '#f0f0ec' }}>
                <th style={{ padding: '6px 16px', textAlign: 'left', color: '#888', fontWeight: 600, fontSize: '9px' }}>TEMPORADA</th>
                {bases.map(b => (
                  <>
              ...
              {seasons.map((season, si) => (
                <tr key={season} style={{ borderTop: '1px solid #f0f0ec', background: si % 2 === 0 ? '#fff' : '#fafaf8' }}>
                  ...
                  {bases.map(base => {
                    ...
                    return (
                      <>
                        <td key={`${base}-pc`} style={{ padding: '7px 12px', textAlign: 'right', color: '#1a1a1a', fontFamily: 'monospace' }}>
                          {rate?.pc_rate != null ? `$${rate.pc_rate}` : <span style={{ color: '#e5e7eb' }}>—</span>}
                        </td>
                        <td key={`${base}-nt`} style={{ padding: '7px 12px', textAlign: 'right', color: '#9ca3af', fontFamily: 'monospace' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '8px', marginBottom: '16px', padding: '12px 16px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {hotel.net_rate_validity && (
            <div>
              <div style={{ fontSize: '9px', color: '#aaa', textTransform: 'uppercase', marginBottom: '2px' }}>Vigencia NT</div>
              <div style={{ fontSize: '12px', color: new Date(hotel.net_rate_validity) < new Date() ? '#ef4444' : '#1a1a1a' }}>{hotel.net_rate_validity}</div>
            </div>
          )}
          {hotel.pc_rate_validity && (
            <div>
              <div style={{ fontSize: '9px', color: '#aaa', textTransform: 'uppercase', marginBottom: '2px' }}>Vigencia PC</div>
              <div style={{ fontSize: '12px', color: '#1a1a1a' }}>{hotel.pc_rate_validity}</div>
            </div>
          )}
          {hotel.closing_date && (
            <div>
              <div style={{ fontSize: '9px', color: '#aaa', textTransform: 'uppercase', marginBottom: '2px' }}>Cierre temporada</div>
              <div style={{ fontSize: '12px', color: '#1a1a1a' }}>{hotel.closing_date}</div>
            </div>
          )}
          {hotel.currency && (
            <div>
              <div style={{ fontSize: '9px', color: '#aaa', textTransform: 'uppercase', marginBottom: '2px' }}>Moneda</div>
        ...
        {promos.length > 0 && (
          <div style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '8px', marginBottom: '16px', overflow: 'hidden' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #f0f0ec', background: '#fafaf8' }}>
              <span style={{ fontSize: '11px', fontWeight: 700, color: '#1a1a1a' }}>PROMOCIONES</span>
            </div>
          ...
          {hotel.contact_email ? (
            <>
              <p style={{ fontSize: '11px', color: '#666', margin: '0 0 12px' }}>
                Se enviará un mail a <strong>{hotel.contact_email}</strong> con botones para que el hotel confirme o rechace.
              </p>
              <Link href={`/hoteles/${params.id}/disponibilidad`} style={{ display: 'inline-block', background: '#1a1a1a', color: '#fff', fontSize: '11px', padding: '8px 16px', borderRadius: '6px', textDecoration: 'none' }}>
                Pedir disponibilidad →
              </Link>
            </>
          ) : (
            <p style={{ fontSize: '11px', color: '#ef4444', margin: 0 }}>Este hotel no tiene email de contacto configurado.</p>
          )}
        </div>
      </div>
    </div>
