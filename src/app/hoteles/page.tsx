...
const REGIONS = [
  ...
  { key: 'CA', label: 'Carretera Austral',  flag: '🛣️',  special: 'CA' },
  { key: 'EX', label: 'Exterior',           flag: '🌎',  countries: ['CL','PE','UY','PY','CO','EC','BO'] },
  { key: 'BR', label: 'Brasil',             flag: '🇧🇷', countries: ['BR'] },
]

const CA_DEST_CODES = ['Caleta Tor','Chaitén','Puelo','Hornopirén','La Junta','Aysen','Puyuhuapi','Coyhaique','Villa C. C','Puerto Tra','Chile Chic','Puerto Gua','Cochrane',"Villa O'Hi"]
  ...
  return (
    <div
      ...
      style={{
        ...
        display: 'grid',
        gridTemplateColumns: '18px 20px 1fr 68px 58px 50px 58px 50px',
        padding: '5px 14px',
        borderBottom: '0.5px solid #ddd8d0',
        background: idx % 2 === 0 ? '#ffffff' : '#f9f6f2',
        alignItems: 'center',
        cursor: 'pointer',
      }}
      onClick={() => onNavigate(hotel.id)}
      onMouseEnter={e => (e.currentTarget.style.background = '#eee8e0')}
      onMouseLeave={e => (e.currentTarget.style.background = idx % 2 === 0 ? '#ffffff' : '#f9f6f2')}
    >
      <div
        {...(isAdmin ? { ...attributes, ...listeners } : {})}
        onClick={e => e.stopPropagation()}
        style={{ color: isAdmin ? '#a89880' : 'transparent', fontSize: '14px', cursor: isAdmin ? 'grab' : 'default', lineHeight: 1 }}
      >⠿</div>
...
export default function HotelesPage() {
  ...
  async function loadData() {
    ...
    if (region === 'CA') {
      destIds = allDests.filter(d => CA_DEST_CODES.includes(d.code)).map(d => d.id)
    } else {
      const regionDef = REGIONS.find(r => r.key === region)
      const countries = (regionDef as any)?.countries ?? []
      destIds = allDests.filter(d => countries.includes(d.country) && !CA_DEST_CODES.includes(d.code)).map(d => d.id)
    }
  ...
  const filteredDests = (() => {
    ...
    if (region === 'CA') {
      list = allDests.filter(d => CA_DEST_CODES.includes(d.code))
    } else {
      const regionDef = REGIONS.find(r => r.key === region)
      const countries = (regionDef as any)?.countries ?? []
      list = allDests.filter(d => countries.includes(d.country) && !CA_DEST_CODES.includes(d.code))
    }
  ...
  async function handleDragEnd(e: DragEndEvent, destId: string, category: string) {
    ...
    const catHotels = getHotels(destId).filter(h => h.category === category)
    const oldIdx = catHotels.findIndex(h => h.id === active.id)
    const newIdx = catHotels.findIndex(h => h.id === over.id)
    if (oldIdx === -1 || newIdx === -1) return
    const reordered = arrayMove(catHotels, oldIdx, newIdx)
  ...
  const C = {
    ...
    topbarBorder: '#ccc5bb',
    colHeader: '#e0d8ce',
    colHeaderBorder: '#c4bbb0',
    colHeaderText: '#6a5e54',
    destHeader: '#d5cdc4',
    destHeaderBorder: '#bdb5ac',
    destText: '#2c2420',
    destMuted: '#8a7d72',
  }

  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, fontFamily: "'Inter','Helvetica Neue',system-ui,sans-serif" }}>

      {/* SIDEBAR */}
      <aside style={{ width: '188px', minWidth: '188px', background: C.sidebar, borderRight: `0.5px solid ${C.sidebarBorder}`, display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        <div style={{ padding: '16px 14px 12px', borderBottom: `0.5px solid ${C.sidebarBorder}` }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: C.destText }}>Say Hueque</div>
        ...
        <nav style={{ padding: '8px 0', flex: 1 }}>
          ...
          {REGIONS.map(({ key, label, flag }) => (
            <button key={key} onClick={() => { setRegion(key); setSearch('') }} style={{
              ...
              color: region === key ? C.destText : C.navMuted,
      ...
      <main style={{ flex: 1, overflow: 'auto' }}>
        ...
        {/* Top bar — sticky */}
        <div style={{ padding: '8px 14px', borderBottom: `1px solid ${C.topbarBorder}`, background: C.topbar, display: 'flex', alignItems: 'center', gap: '10px', position: 'sticky', top: 0, zIndex: 20 }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: '340px' }}>
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#a09080', fontSize: '13px', padding: 0 }}>✕</button>
            )}
          </div>
        ...
        {/* Column headers — sticky below topbar */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: '18px 20px 1fr 68px 58px 50px 58px 50px',
          padding: '6px 14px',
          background: C.colHeader,
          borderBottom: `1px solid ${C.colHeaderBorder}`,
          position: 'sticky',
          top: '41px',
          zIndex: 19,
        }}>
          ...
          return (
            <div key={dest.id}>
              ...
              <div style={{ padding: '7px 14px', background: C.destHeader, borderBottom: `1px solid ${C.destHeaderBorder}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 700, color: C.destText }}>{dest.name}</span>
                ...
                return (
                  <div key={cat}>
                    <div style={{ padding: '3px 14px 3px 52px', background: cs.bg, borderBottom: `0.5px solid ${cs.text}30`, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ fontSize: '10px', fontWeight: 700, color: cs.text, background: cs.pill, padding: '1px 8px', borderRadius: '20px', letterSpacing: '0.04em' }}>{cat}</span>
